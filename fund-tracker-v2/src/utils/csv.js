...
export function validateCSVRow(row, rowIndex) {
  ...
  const dailyRateText = String(row.jiaDailyRate ?? row.dailyRate ?? "").trim();
  ...
  return {
    record: {
      id: createId(),
      date: normalizedDate,
      dailyRate: getCalculatedRate(Number(dailyRateText)) / 100,
      deposit,
      withdraw,
      note
    }
...
export function downloadCSVTemplate() {
  const template = [
    "date,jiaDailyRate,deposit,withdraw,note",
    "2026/3/4,1.00,100,0,示例：首次买入，参与计算为0.500%",
    "2026/3/5,-0.50,0,0,示例：下跌，参与计算为-0.250%",
    "2026/3/6,0.80,100,0,示例：定投，参与计算为0.400%"
  ].join("\n");
